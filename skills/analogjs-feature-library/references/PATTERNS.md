...
- [Pattern 6: Advanced Feature Library (Project Standard)](#pattern-6-advanced-feature-library-project-standard)
...
## Pattern 6: Advanced Feature Library (Project Standard)

**Purpose:** Granular, scalable feature architecture for complex domain logic.

**Use Case:** Core features like `guest-management`, `budget-management`, etc.
...
├── src/
│   ├── index.ts                # Client-side public API
│   ├── server.ts               # Server-side public API
│   ├── lib/
│   │   ├── data-access/        # HTTP services, repositories
│   │   ├── store/              # Signal-based stores (ngrx/signals)
...
- **`index.ts`**:
  ```typescript
  export * from './lib/data-access/feature.service';
  export * from './lib/store/feature.store';
  export * from './lib/types/feature.model';
  ```
- **`server.ts`**:
  ```typescript
  export * from './backend';
  export * from './lib/data-access/feature.repository';
  export * from './lib/models/feature.schema';
  ```

### Key Points

- **Granular `lib/`**: Avoid putting all components in `components/`. Separate UI from Page logic.
- **Signal Stores**: Use `@ngrx/signals` for state management in `store/`.
- **Double Barrel**: Use `index.ts` for frontend and `server.ts` for backend to prevent leakage of server-only code (like Mongoose) to the frontend.
